...
% Parameters:
min_len = 3*60; % [sec]
timit_folder = 'D:\Thesis\Databases\TIMIT\timit\TIMIT\TRAIN';
% musan_folder = 'C:\meital\University\Madison\Thesis\Database\musan\noise';
noise_folder = 'D:\Thesis\Databases\General6000';
...
%     right_win = (1:-1/win_len:0)'; % (right)
    ...
    num_noise = 6000; % choose only 6000 noises from the database
...
%-----------------------------------
% step 1.1: create long noise recs for babble noise only:
%-----------------------------------
if stage <= 1.1 && exit_stage>= 1.1
    A = 1; % amplitude of noise 
...
%     noise_list = noise_list(randperm(numel(noise_list))); % shuffle list
    ...
    % open babble_file to get lines of babble noise:
    bfile = 'C:\meital\University\Madison\Thesis\Database\babble_list.xlsx';
    blist = xlsread(bfile);
    
    % open file info excel:
    inffile = 'D:\Thesis\Databases\General6000\__6000 documents\6000-HD-combo-contents-mar2017.xls';
    [n,inflist,r] = xlsread(inffile);
    
        % create noise recs 
    for i=1:length(blist)
        ind = blist(i);
        file_name = cell2mat(inflist(ind,2));
        Index = find(contains(noise_list,file_name));
        file_i = cell2mat(noise_list(Index));
        [sound_i, fs] = audioread(file_i); % get noise signal
    ...
    wav_name = ['noise_train',repmat('0',1,(3-length(num2str(file_count)))),num2str(file_count),'.wav'];
    audiowrite([noise_train,'\',wav_name],rec,Fs); % save audio to valid folder
    minute_counts
end
%-----------------------------------
