...
out_train = 'C:\meital\University\Madison\Thesis\VAD-py\data\raw\train';
out_valid = 'C:\meital\University\Madison\Thesis\VAD-py\data\raw\valid';
noise_train = 'C:\meital\University\Madison\Thesis\VAD-py\data\raw\noise\train';
noise_valid = 'C:\meital\University\Madison\Thesis\VAD-py\data\raw\noise\valid';
stage = 3;
exit_stage = 3;
...
%     right_win = (1:-1/win_len:0)'; % (right)
    ...
    num_noise = 6000; % choose only 5500 noises from the database
    train_num = num_noise*0.95; % number of files for the train folder (~95%)
    ...
    for i=1:num_noise
        ...
        if length(sound_i) >= min_len*Fs 
            ...
            for j = 1:in
                wav_name = ['noise_train',repmat('0',1,(3-length(num2str(file_count)))),num2str(file_count),'.wav'];
                ...
                if i <= train_num  % put wav in out_train     
                    audiowrite([noise_train,'\',wav_name],sound_save,Fs); % save audio to train folder 
                else % put wav in out_valid
                    audiowrite([noise_valid,'\',wav_name],sound_save,Fs); % save audio to train folder 
                end
        ...
        if length(rec)+length(sound_i) >= min_len*Fs 
            if isempty(rec) == 0 % make sure rec is not empty
                ...
                if i <= train_num  % put wav in out_train     
                    audiowrite([noise_train,'\',wav_name],rec,Fs); % save audio to train folder 
                else % put wav in out_valid
                    audiowrite([noise_valid,'\',wav_name],rec,Fs); % save audio to train folder 
                end
                rec = [];
                file_count = file_count+1;
            end
    ...
    wav_name = ['noise_train',repmat('0',1,(3-length(num2str(file_count)))),num2str(file_count),'.wav'];
    audiowrite([noise_valid,'\',wav_name],rec,Fs); % save audio to valid folder
    minute_counts
end
...
%     utt_data = utt_data(randperm(numel(utt_data))); % shuffle utt_data
    ...
    noise_train_list = getAllFiles(noise_train, 'FileFilter','\.wav$','Depth',1);
    ...
    for i=1:length(noise_train_list)
        ...
        while length(new_signal) < length(noise_i)
            ...
            if length(utt_j) > length(noise_i)-length(new_signal)
                noise_cut = noise_i(1:length(new_signal));
                noisy_signal = v_addnoise(new_signal,Fs,snr_i,m,noise_cut);
                
                audio_name = strrep(strrep(rec_i,'noise_','timit_'),'\noise\','\');
                noisy_signal = noisy_signal/max(abs(noisy_signal));
                audiowrite(audio_name,noisy_signal,Fs); % save audio to train folder
                minute_counts = minute_counts + length(noisy_signal)/Fs/60;
            ...
            % normalize and add utt to signal
            if isempty(new_signal) ~= 1
                utt_e = sum(utt_j.^2)/length(utt_j);
                new_signal_e = sum(new_signal.^2)/length(new_signal);
                utt_j = sqrt(new_signal_e/utt_e)*utt_j;
            end
            new_signal = [new_signal;utt_j];
    ...
    % Valid data:
    %---------------
    % Get all noise recs from valid directory:
    noise_valid_list = getAllFiles(noise_valid, 'FileFilter','\.wav$','Depth',1);
    file_count = file_count+length(noise_valid_list);
    
    
    % Run through the noise_valid_list:
    for i=1:length(noise_valid_list)
        rec_i = cell2mat(noise_valid_list(i));
        [noise_i,Fs] = audioread(rec_i);
        
        new_signal = []; % new recording of noice and utterances + silence
        y_label = []; % new transcript to the recording
        % choose snr for recording
        snr_i = choose_snr(snr_list,length(noise_valid_list),i);
...
%         snr_i = snr_list(rand_ind);
        ...
        while length(new_signal) < length(noise_i)
            ...
            if length(utt_j) > length(noise_i)-length(new_signal)
                noise_cut = noise_i(1:length(new_signal));
                noisy_signal = v_addnoise(new_signal,Fs,snr_i,m,noise_cut); % add noise and utt
    ...
    minute_counts
%     %------------------------
%     % Leftovers: make recordings with the left utterances + white noise
%     %------------------------
%     new_signal = []; % new recording of noice and utterances + silence
%     y_label = []; % new transcript to the recording
%     file_count = length(noise_train_list)+length(noise_valid_list)+1;
%     rand_ind = randi(length(snr_list), 1);
%     snr_i = 12;
%     
%     for i=j:length(utt_data)
...
%         
%         if length(new_signal)+length(utt_i) >= min_len*Fs 
%             if isempty(new_signal) == 0 % make sure rec is not empty
%                 audio_name = [out_train,'\timit_train',repmat('0',1,(3-length(num2str(file_count)))),num2str(file_count),'.wav'];
%                 new_signal = new_signal/max(abs(new_signal));
%                 audiowrite(audio_name,new_signal,Fs); % save audio to train folder
%                 save(strrep(audio_name,'wav','mat'),'y_label')
%                 silence_count = silence_count + (length(noise_i)-length(new_signal));
%                 new_signal = [];
%                 y_label = [];
%                 rand_ind = randi(length(snr_list), 1);
%                 snr_i = snr_list(rand_ind);
...
%             end
%         end
%         trans = [silence;ones(length(speech(b:e)),1);silence]; % transcript
%         speech_count = speech_count+length(speech(b:e));
%         silence_count = silence_count+length(silence)*2;
%         
%         z = v_addnoise(utt_i,Fs,snr_i); % add noise and utt
%         new_signal = [new_signal;z];
%         y_label = [y_label;trans];
%     end
%     audio_name = [out_train,'\timit_train',repmat('0',1,(3-length(num2str(file_count)))),num2str(file_count),'.wav'];
%     new_signal = new_signal/max(abs(new_signal));
%     audiowrite(audio_name,new_signal,Fs); % save audio to train folder
